...
class Stock(productId: Long, quantity: Int) : BaseEntity() {
    ...
    init {
        require(productId > 0) {
            throw CoreException(BAD_REQUEST, "유효하지 않은 상품 ID 입니다. 상품 ID: $productId")
        }
        require(quantity >= 0) {
            throw CoreException(BAD_REQUEST, "재고 수량은 0 이상이어야 합니다. 현재 수량: $quantity")
        }
    }
    ...
    fun deduct(quantity: Int) {
        require(quantity > 0) {
            throw CoreException(BAD_REQUEST, "차감할 수량은 0보다 커야 합니다. 요청 수량: $quantity")
        }
